...
export async function optimizeCmsImageUploads(files) {
    await Promise.all(files.map((file) => optimizeCmsImageUpload(file)));
}
export async function optimizeCmsImageUpload(file) {
    if (!file.mimetype.startsWith('image/') || !['projectImages', 'workLogo'].includes(file.fieldname))
        return;
    ...
    try {
        ...
        if (isProjectImage) {
            ...
            await sharp(inputPath, { failOn: 'warning', limitInputPixels: 80000000 })
                .rotate()

...
const NUB_TRAITS = { shape: 0.74 };
const LOVEY_RE = /\b(love|loves|loved|adore|adores|adorable|cute|cutie|sweet|sweetheart|aw+h*|handsome|pretty|beautiful|crush|lovely|precious|so nice|i like (him|you|this)|you('re| are) (cute|sweet|lovely|amazing|adorable))\b|❤️|💕|💖|💗|😍|🥰|😘/;
const BLUSH_RE = /\b(funny|funniest|cool|awesome|amazing|impressive|talented|smart|brilliant|epic|legendary|goat|dope|wonderful|fantastic|incredible|inspiring|clever|witty|charming|excellent|so cool|so funny|you('?re| are) (smart|the best)|he('?s| is) the best|damn you'?re?)\b/;
/** Ranking / “who’s better” questions — praise words, not compliments. */
const RANKING_RE = /\b(who('?s| is)|which|between)\b.{0,60}\b(best|better|worse|worst|goat)\b|\b(best|better)\b.{0,40}\b(me|you|him|or|darkguyaiman)\b/i;
const JOKE_RE = /\b(joke|jokes|joking|pun|punny|dad\s*joke|dirty joke|sex joke|lmao+|rofl|knock[\s-]?knock|why did the|why don'?t(?:cha| you)?|what'?s?\s+another\s+name|what do you call|what is another name|another name (for|of)|another word for|what'?s another|what is .{0,40} (afraid of|called|for|of)|afraid of|what'?s the difference|how many .{0,40} does it take|walks into a(?:n)? bar|here'?s one|hear this|did you hear)\b/i;
const JOKE_FOLLOWUP_RE = /^(nope|no+|nah|actually|wrong|nuh\s*uh|it'?s|the (answer|punchline) is|you (got|missed)|come on)\b/i;
const WAITING_FOR_PUNCHLINE_RE = /\b(i'?m blanking|don'?t get it|no idea\.|i'?m lost|hit me with it|what'?s the punchline|tell me\.?$)\b/i;
const WANTS_JOKE_TOLD_RE = /\b(tell( me)? (a |another |one more )?joke|got (any |a )?jokes?|make me laugh|say a joke|know any jokes?)\b/i;
const WANTS_JOKE_EXPLAINED_RE = /\b(explain( the| this| that)? joke|why (is |was )?(that|it|this) funny|why did (it|that) get you|how do you understand|what does .{0,60} mean|why (did|does) (the )?joke|walk me through (the |that )?joke|in detail|more detail|more details|elaborate|go deeper|break it down|explain (it|that|this) more|say more)\b/i;
/** Adult / sex-humour cues (riddles & standup-style bits — not a joke database). */
const SEX_HUMOUR_RE = /\b(sex|sexy|sexual|boobs?|breasts?|penis|vagina|willy|dick|pussy|orgasm|cum+ing|coming in|dishwasher|blow ?job|naked|nude|bedroom|horny|intercourse|married|sucking|licking|ice cream parlor|jewelry)\b/i;
...
function wantsJokeExplained(text, history = []) {
    const t = String(text || '').toLowerCase().trim();
    if (!t)
        return false;
    if (WANTS_JOKE_EXPLAINED_RE.test(t))
        return true;
...
function looksLikeJokeSetup(text) {
    ...
    if (!t)
        return false;
    if (wantsJokeTold(t) || wantsJokeExplained(t))
        return false;
    if (isJokeText(t))
        return true;
    ...
    if (/^(what'?s?|what is|why|how|who|did you hear|so )\b/.test(t) && t.length <= 200) {
        ...
        if (/\b(get you|funny|understand|explain|mean)\b/.test(t))
            return false;
    ...
    if (t.length <= 160 && isSexHumour(t) && /\?/.test(t))
        return true;
    return false;
}
...
function isToldFullJoke(message) {
    ...
    if (wantsJokeTold(t) || wantsJokeExplained(t))
        return false;
    if (!isSexHumour(t) && !/\b(joke|said|asks?|replied|teacher|wife|husband)\b/i.test(t))
        return false;
    return /["“]|said|asks?|replied|then |finally |surprise/i.test(t);
}
function assistantWaitingForPunchline(history) {
    for (let i = history.length - 1; i >= 0; i -= 1) {
        const turn = history[i];
        if (!turn)
            continue;
        if (turn.role === 'assistant')
            return WAITING_FOR_PUNCHLINE_RE.test(turn.content);
        if (turn.role === 'user')
            return false;
    }
    return false;
...
function isJokeSetup(message, history = []) {
    if (wantsJokeTold(message) || wantsJokeExplained(message, history))
        return false;
    if (isToldFullJoke(message))
        return false;
 ...
 */
function isJokePunchline(message, history = []) {
    if (wantsJokeTold(message) || wantsJokeExplained(message, history))
        return false;
    if (isLoveyText(message) || isBlushText(message))
        return false;
    if (isToldFullJoke(message))
        return true;
    ...
    if (!assistantWaitingForPunchline(history))
        return false;
    // Don't treat a brand-new setup as the punchline.
    if (looksLikeJokeSetup(trimmed) && !JOKE_FOLLOWUP_RE.test(trimmed))
        return false;
    if (JOKE_FOLLOWUP_RE.test(trimmed))
        return true;
...
function paletteFor(expressionName) {
    if (expressionName === 'love' || expressionName === 'shy') {
        return { ...LOVE_PALETTE };
    }
    return { ...BROWN_PALETTE };
}
function pickExpression(message, reply, mode = 'none') {
    const lowerReply = reply.toLowerCase();
    if (mode === 'punchline')
        return 'happy';
    if (mode === 'setup')
        return 'unsure';
    if (isLoveyText(message))
        return 'love';
    if (isBlushText(message))
        return 'shy';
    ...
    if (/sorry|unfortunately/.test(lowerReply))
        return 'sad';
    return 'happy';
}
const rateBuckets = new Map();
let promptCache = null;
const PROMPT_TTL_MS = 60000;
...
function publicAge(asOf = new Date()) {
...
function renderAnimatedAvatar(seed, expression, size, expressionName = 'idle') {
...
function renderStaticAvatar(seed, expression, size, expressionName = 'idle') {
...
async function buildSystemPrompt() {
    ...
    try {
        ...
        experience = (work || []).slice(0, 12).map((item) => (`- ${item.role} at ${item.company} (${item.startDate}–${item.endDate})`));
...
function jokePunchlineFallback(message) {
    ...
    if (bit)
        return `Okay — "${bit}" — that got me. 😂`;
    return `Okay okay — you got me. 😂`;
}
function jokeExplainFallback(history) {
...
function looksLikeWeakJokeExplain(reply) {
...
async function findMentionedProject(message) {
...
function portfolioProjectFallback(project) {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const expression = pickExpression(message, reply, jokeTell ? 'punchline' : 'none');
        return res.json({
            reply,
            expression,

...
function fingerprintPublicAssets(root: string): string {
  const files = [
  ...
  for (const file of files) {
    ...
    hash.update(file);
    if (fs.existsSync(fullPath)) hash.update(fs.readFileSync(fullPath));
  }
...
const publicErrorCopy: Record<number, PublicErrorCopy> = {
  400: {
...
function renderPublicError(req: Request, res: Response, requestedStatus: number) {
...
app.use((error: PublicHttpError, req: Request, res: Response, next: NextFunction) => {
...
function warmupProjectAssets(projects: Project[]) {

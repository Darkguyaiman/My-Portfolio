...
import {
  ...
  getAdminWork,
  getDashboardStats,
  getNextEducationDisplayOrder,
  getNextLanguageDisplayOrder,
  getNextProjectDisplayOrder,
  getNextWorkDisplayOrder,
  getSiteContent,
  listAdminEducation,
  listAdminLanguages,
  listAdminProjects,
...
router.get('/work/:id/edit', async (req, res, next) => {
  try {
    ...
    if (!work) return res.redirect('/admin/work');
...
router.post('/work/:id', cmsUploads, async (req, res, next) => {
  try {
    ...
    if (!existing) return res.redirect('/admin/work');
...
router.get('/education/:id/edit', async (req, res, next) => {
  try {
    ...
    if (!education) return res.redirect('/admin/education');
...
router.post('/education/:id', async (req, res, next) => {
  try {
    ...
    if (!existing) return res.redirect('/admin/education');
...
router.get('/languages/:id/edit', async (req, res, next) => {
  try {
    ...
    if (!language) return res.redirect('/admin/languages');
...
router.post('/languages/:id', async (req, res, next) => {
  try {
    ...
    if (!existing) return res.redirect('/admin/languages');
...
function parseReorderIds(body: Record<string, unknown>) {
  return Array.isArray(body.ids)
    ? body.ids.map((value: unknown) => Number(value)).filter((value: number) => Number.isFinite(value) && value > 0)
    : [];
}

function parseProject(body: Record<string, unknown>, uploads: UploadedPaths = {}) {
...
function parseWork(body: Record<string, unknown>, uploads: UploadedPaths = {}) {
  return {
...
function parseEducation(body: Record<string, unknown>) {
  return {
...
function buildEducationResults(body: Record<string, unknown>) {
  ...
  if (semester !== null) results.semester = semester;
  if (gpa !== null) results.gpa = gpa;
  if (totalSubjects !== null || Object.keys(filteredGrades).length > 0) {
    results.total_subjects = totalSubjects ?? Object.values(filteredGrades).reduce<number>((total, count) => total + Number(count), 0);
...
function parseLanguage(body: Record<string, unknown>) {
  return {
...
function parseSiteContent(body: Record<string, unknown>, uploads: UploadedPaths = {}): SiteContent {
  return {
...
type UploadedPaths = Record<string, string[]>;

function getUploadedPaths(req: Request): UploadedPaths {
